...
- **Needs attention.** Wines whose window closes within a year or has passed get a badge on the Cellar tab and a **Needs attention** chip beside the type filters — tap it to see just those, most urgent first.
- **Farms.** A record of the wine farms you've visited: check in to capture the GPS location (works offline), add a photo, a star rating out of 5 (half-stars supported), the date and a review. Farms sort by newest visit (tap the sort button's selected option to reverse, or switch to rating). A location can also be added later by pasting coordinates — in Google Maps, press and hold the spot, copy the numbers, paste them into the farm's Location box. **Open in Maps** drops you on the pin, and **Share** sends the farm — name, place, rating, date, your review and a Maps link — to WhatsApp or anywhere else via the phone's share sheet. Farms added without a check-in still work: the link searches Maps by name instead.
...
- **Export backup** downloads a `.json` file instead — handy on a laptop. Backups are self-contained: photos travel inside the file, so a restore on a new phone brings everything back.
